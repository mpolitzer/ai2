...
int *a_star_search(int init[2], int goal[2], float *_distance, int *_steps)
{
	...
	while(curr && (curr->pos[0] != goal[0] || curr->pos[1] != goal[1])) {
		for(i = 0; i < 4; i++) {
			...
			/* this way has a wall */
			if(mapw[map_index] == MAP_WALL) continue;

			/* calculating total distance */
			d = curr->distance + mapw[map_index]==MAP_ROAD?1:0;
	...
	if(curr)
	{
		...
		*_steps = curr->steps;
		...
		final_path = malloc(sizeof(int)*(curr->steps));
	}

	/* steal the nodes we'll need from _map */
	for(; curr->prev; curr=curr->prev) {
		int dx = curr->pos[0]-curr->prev->pos[0];
		int dy = curr->pos[1]-curr->prev->pos[1];
		if(dx == -1)
			final_path[final_idx] = DIR_W;
		else if(dx == 1)
			final_path[final_idx] = DIR_E;
		else if(dy == -1)
			final_path[final_idx] = DIR_N;
		else if(dy == 1)
			final_path[final_idx] = DIR_S;
		final_idx--;
	}
